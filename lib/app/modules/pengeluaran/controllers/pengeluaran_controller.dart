...
class PengeluaranController extends GetxController {
  ...
  var pengeluaranBulanIni = 0.0.obs;

  // Data transaksi pengeluaran terbaru
  ...
  final TotalSaldoController saldoController = Get.find<
      TotalSaldoController>(); // Akses controller global
  ...
  Future<void> getUserData() async {
    try {
      ...
      if (uid.isNotEmpty) {
        ...
        await FirebaseFirestore.instance.collection('data_profile')
            .doc(uid)
            .get();
        if (userDoc.exists) {
  ...
  void addTransaction(String name, String date, double amount) {
    ...
    // Kurangi amount dari pengeluaran bulan ini dan total saldo
    pengeluaranBulanIni.value += amount;
    saldoController.totalSaldo.value -= amount; // Perbarui saldo global

    print('Total Saldo: ${saldoController.totalSaldo.value}');
    print('Pengeluaran Bulan Ini: ${pengeluaranBulanIni.value}');
    update();
  }
  ...
  void updateTransaction(int index, String name, String date, double amount) {
    ...
    // Update pengeluaran bulan ini dan total saldo
    pengeluaranBulanIni.value += amount - oldAmount;
  ...
  void deleteTransaction(int index) {
    ...
    // Kurangi amount dari pengeluaran bulan ini dan total saldo
    pengeluaranBulanIni.value -= amountToRemove;
  ...
  void editData(int index, double newAmount) {
    ...
    pengeluaranBulanIni.value -= oldAmount;
    pengeluaranBulanIni.value += newAmount;
  ...
  void deleteData(int index) {
    ...
    pengeluaranBulanIni.value -= removedAmount;
